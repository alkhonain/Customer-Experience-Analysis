...
values[1]
values <- Corpus(VectorSource(documents$content))
values <- tm_map(values, stemDocument)
print("After Stemming")
print(values$content[1])
values$content <- tokenize(values$content)
```


prototype of word impact on review, (NOT COMPLETED)
```{python}
occurences = {}
for run in r.values["content"][0:100]:
...
for word in list(occurences.keys())[0:100]:
  print(word, occurences[word][0])
```
